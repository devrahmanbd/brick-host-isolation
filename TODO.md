...
- [x] Repaired the broker adversarial-test synchronization defect discovered during Phase 51 validation; preserved fail-closed broker behavior and passed `go test -race` deterministically.
...
- [x] Remediated the SR-001 code boundary by cryptographically binding signed Phase 8 evidence to Phase 9 candidate identity, rejecting mismatches, adding deterministic adversarial coverage, standalone verifier coverage, reciprocal Core compatibility fixtures, and fail-closed CI gates.
- [ ] **Blocked pending protected signers and real hosts:** Regenerate both Shared/Dedicated real-host evidence records with the Phase 51 binding and obtain an independent re-review before any approved security review is requested.
...
- [x] Define the certificate-gated Shared/Dedicated pin and deployment-change procedure in `release/EDITION_PIN_HANDOFF_v1.0.0.md` without creating a placeholder pin or approval.
- [ ] **Blocked pending independently verified guarded-release certificate:** Create Shared and Dedicated immutable pin records and approved deployment-change records only after independent verification of a valid unexpired Phase 9 guarded-release certificate. Each pin must bind the release ID, exact source commit, certificate digest, artifact/SBOM/benchmark/review digests, edition-specific staging-evidence digest, three edition-specific Core-gate digests, verifier receipt, and protected audit correlation. Do not create placeholder pins or approvals.
- [ ] Run a post-pin revoke/rollback drill only on an independently controlled non-customer staging cage after a valid pin exists: revoke an isolated signer or expire an isolated review, prove new pin authorization halts, route the disposable cage through Phase 7 kill-first recovery, and retain a protected incident/audit/remediation record. Do not simulate against customer or production resources and do not substitute local fixtures for real recovery evidence.
- [x] Define the safe post-pin review-expiry/signer-revocation and Phase 7 recovery drill in `release/POST_PIN_REVOKE_RECOVERY_DRILL_HANDOFF_v1.0.0.md` without creating a false incident record or touching customer/production resources.
- [ ] **Blocked pending valid pin and independent staging cage:** Execute the protected post-pin drill and retain its signed revocation/expiry, halt-proof, Phase 7 journal/audit/evidence/attestation, and independent remediation confirmation outside source control.
...
- [x] Make the Phase 2 oversized-frame regression deterministic by asserting on an oversized declared frame length rather than relying on a payload write that races the server close; retain proof of the malformed-frame denial and audit event. Focused race test and full Phase 9 gate passed.
